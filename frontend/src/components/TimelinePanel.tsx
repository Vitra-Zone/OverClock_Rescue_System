...
const ACTOR_CONFIG = {
  ...
  ai:     { icon: <Bot size={12} />,          color: 'text-crisis-primary', bg: 'bg-orange-900/40', label: 'AI Agent' },
          ...
          <div key={i} className="flex gap-3 animate-fade-in">
            ...
            <div className="flex-1">
              <div className="flex items-baseline gap-2 mb-0.5">
                ...
                <span className="flex items-center gap-0.5 text-crisis-muted text-xs">
                  <Clock size={10} /> {formatTime(entry.timestamp)}
                </span>
              </div>
              <p className="text-crisis-text text-sm leading-snug">{entry.message}</p>
            </div>
          </div>
        );
      })}
