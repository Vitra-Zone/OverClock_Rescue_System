...
import { fetchTouristIncidents, fetchIncidents, updateTouristLocation } from '../api/client';
...
export function TouristIncidentsPage() {
  ...
  // Load incidents — prefer tourist-specific endpoint when authenticated, otherwise fall back to public incidents.
  useEffect(() => {
    let mounted = true;
    const load = async () => {
      setBusy(true);
      try {
        const data = profile?.uid ? await fetchTouristIncidents() : await fetchIncidents();
        if (!mounted) return;
        setIncidents(data ?? []);
      } catch (err) {
        if (!mounted) return;
    ...
    // only load once per mount (or when auth state changes)
    void load();
    return () => { mounted = false; };
  }, [profile?.uid, loading]);

  // Keep center in sync with profile when available
  useEffect(() => {
  ...
  const refreshPosition = async () => {
    ...
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      try {
        // Try to write location if authenticated; otherwise just update local center.
        if (profile?.uid) {
        ...
        console.warn('[TouristIncidentsPage] Failed to update location to server', err);
      } finally {
    ...
    <div className="min-h-screen bg-[#0b1020] text-white flex flex-col">
      <div className="px-3 pt-3 sm:px-4 sm:pt-4">
        <div className="flex items-center gap-3">
          <button onClick={() => void refreshPosition()} disabled={refreshing} className="rounded-2xl border border-white/10 bg-[#10182e]/85 px-3 py-2 backdrop-blur text-sm inline-flex items-center gap-2 text-slate-100">
          ...
          <aside className="col-span-1 p-4 space-y-3">
            <div className="rounded-xl border border-white/6 bg-[#0f1724]/60 p-3">
              ...
              <div className="mt-2 max-h-[60vh] overflow-y-auto">
                {incidents.length === 0 ? (
                  <div className="text-sm text-slate-400">No incidents found nearby.</div>
                ) : incidents.map((inc) => (
                  <button key={inc.id} onClick={() => { setSelectedIncident(inc); setCenter(inc.coordinates ?? center); setRecenterToken((v) => v + 1); }} className={`w-full text-left p-2 rounded-md hover:bg-white/5 ${selectedIncident?.id === inc.id ? 'ring-2 ring-orange-400' : ''}`}>
                    <div className="flex items-center justify-between">
                      <div className="font-medium text-sm">{inc.incidentType.toUpperCase()}</div>
                      <div className="text-xs text-slate-400">{inc.coordinates ? `${calculateDistanceKm(center, inc.coordinates).toFixed(1)} km` : '—'}</div>
                    </div>
                    <div className="text-xs text-slate-300">{inc.location}</div>
                  </button>
                ))}
              </div>
            </div>
